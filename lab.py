...
def get_data_path() -> str:
    ...
    path to the curated tech_news_qa.csv.
...
def token_f1(pred: str, gold: str) -> float:
    ...
    ground_truth_tokens = normalize_answer(gold).split()
    
    if len(prediction_tokens) == 0 or len(ground_truth_tokens) == 0:
        return 1.0 if len(prediction_tokens) == len(ground_truth_tokens) else 0.0
